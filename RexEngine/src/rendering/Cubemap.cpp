...
{		
	...
	Cubemap::Cubemap(const std::string& right, const std::string& left,
		...
		const std::string& front, const std::string& back, RenderApi::PixelFormat format)
	{
		...
		for (int i = 0; i < 6; i++)
		{
			...
			if (data)
				RenderApi::SetCubemapFace(m_id, (RenderApi::CubemapFace)i, format, size, data, format, RenderApi::PixelType::UByte);
			else
	...
	std::shared_ptr<Cubemap> Cubemap::FromHDRI(const std::string& path, Vector2Int size)
	{
		...
		if (hdrMap)
		{
			...
			cubemap->SetDefaultTextureOptions();
	...
	std::shared_ptr<Cubemap> Cubemap::CreateIrradianceMap(const Cubemap& from, Vector2Int size, float sampleDelta)
	{
		auto cubemap = std::make_shared<Cubemap>();
		auto cubemapId = cubemap->GetId();
	...
	void Cubemap::SetDefaultTextureOptions()
	{
